...
## Install Notes (this repo)

Vendored from an upstream repo; see `PROVENANCE.md`.

**This skill targets React. This repository does not use React** — it is static
HTML/CSS/JS with no build step and no `package.json`, and the root `CLAUDE.md`
asks to keep it dependency-free. Nearly every example below is JSX and does not
apply to the current site as written. Treat this skill as reference for a future
React project, or as concept guidance (spring parameters, gesture semantics,
stagger orchestration) to translate by hand into `assets/css/motion.css` and
`assets/js/motion.js`. Do not introduce React, Vite, or a bundler here to use it.

For the vanilla-JS build of the same library, the imports are:

```js
import { animate, scroll, inView, stagger } from "https://cdn.jsdelivr.net/npm/motion@latest/+esm"
```

Note the package split: this skill's examples and its starter template import
from **`framer-motion`** (v11), the older package name. The current package is
**`motion`**. The React API shown here is still valid, but a new project should
prefer `motion`.

**Two files the Resources section claims are not bundled:**

| Claimed | Status |
|---|---|
| `references/api_reference.md` | Present (1077 lines) |
| `references/variants_patterns.md` | **Missing** — never shipped upstream |
| `references/gesture_guide.md` | **Missing** — never shipped upstream |
| `assets/examples/` | Present, as a single 74KB `README.md` |

Do not cite the two missing files. Variant and gesture material is covered in
this file and in `assets/examples/README.md`.
...
**Known upstream bug in `animation_generator.py`.** For `--type stagger`,
`variant`, and `layout`, standalone variant objects are emitted with doubled
braces — a leaked Python format-string escape:

```js
const container = {
  hidden: {{ opacity: 0 }},   // SyntaxError: Unexpected token `{`
}
```

Collapse `{{` / `}}` to `{` / `}` in any *plain JS object literal* it produces.
Note this does **not** apply to JSX props: `initial={{ opacity: 0 }}` is correct
JSX — an expression slot containing an object literal — and must be left alone.
The other eight `--type` values and every `variant_builder.py --preset` are
unaffected.
