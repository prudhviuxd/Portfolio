...
The search script lives inside this skill's own directory. Invoke it from the repository root (the path below is relative to that root); if the working directory is elsewhere, prefix it with the absolute repo path:
