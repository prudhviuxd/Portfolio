...
Vendored from an upstream plugin; see `PROVENANCE.md`. The token-architecture
and component-spec guidance works as-is. The **Slide System** section below,
however, assumes upstream project files that do not exist in this repo:
...
| `/slides:create` command | **Not available** — an upstream plugin command, not part of this skill |
| Skill dependencies `brand`, `ui-styling` | **Not installed** — separate upstream skills |

Do not cite any of the above as if present. Create the missing token files
first, or treat the slide workflow as unavailable and say so.
